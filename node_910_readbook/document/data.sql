...
-- DB 명령을 실행하기 전에 MqSQL에서는 반드시 SCHEMA를 연결(open,use) 해야한다

USE bookdb2;

-- MySQL 에서 사용하는 특별한 명령
-- 기존의 Table을 DROP하고 다시 그 구조대로 CREATE TABLE를 실행한다
...
일부 다른 DB 소프트웨어는 임시기억장소에 아예 데이터를 저장해두고, 어떤 명령등이 실행되면 
한꺼번에 데이터를 저장소에 저장하기도 한다

이러한 중간의 임시장치를 Buffer 또는 cache 라고 한다
이 때 여러곳의 client가 동시에 DB의 데이터에 접근할 경우 일부 client에서는
INSERT, UPDATE, DELETE가 반영되지 않은 데이터를 보게(SELECT) 될 수도 있다
...
-- SET @autocommit = 0;
/*
DDL(Data Definition Lang.) : DRA(Database Administrator, 최고관리자)가 사용하는 명령어
...
-- DB의 물리적 저장소를 생성, 제거, 변경할 수 있는 권한을얻는다
-- 일반적으로 협업에서는 root 사용자 외에 별도의 사용자를 생성하여 제한적인 권한을 부여하여 사용
CREATE USER 'callor'@'localhost' IDENTIFIED BY 'jjh123';
...
무결성을 보장하기 위하여 생성된 사용자에게 적절한 권한을 제한적으로 부여하여 
    여러 민감한 데이터의 손상, 변화(INSERT, UPDATE, DELETE) 를 최소화 하여야 한다
*/

-- callor 사용자에게 bookDB2 데이터베이스에 대하여 권한을 부여하기
-- TABLE 생성, CRUD를 수행할 수 있다
GRANT ALL PRIVILEGES ON bookdb2.* TO 'callor'@'localhost';
